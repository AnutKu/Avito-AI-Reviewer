...
class FraudDecisionPayload(BaseModel):
    ...
    # Обоснование было обязательным и вводилось руками. Форму убрали: вердикт
    # выносится по видимому на экране — индексу с раскладкой и ответам на
    # вопросы, — и переписывание этого в текстовое поле оставалось ритуалом.
    # Поле сохранено и принимает текст: старые решения его показывают, а
    # ограничение длины — обычный предел на вход, а не требование заполнить.
...
def send_blitz(
    ...
    db.add(
        Notification(
            ...
            kind="blitz",
            # Так этот раздел называется в кабинете студента: уведомление ведёт
            # именно в него, и название должно совпадать с пунктом меню.
            title="Вопросы от ревьюера по вашей работе",

...
def _demo_feedback(title: str) -> str:
    """Обратная связь архивной работы — тем же текстом, каким её пишет ревьюер.

    Стояла заглушка в одну строку («Обратная связь опубликована ревьюером»), и
    у студента экран результата выглядел так, будто до него ничего не доехало.
    Демо-данные обязаны показывать форму настоящего ответа: абзац, перечисление
    через «- », абзац о следующем шаге — ровно то, что описано в контракте
    draft_feedback и что увидит студент на живой работе.
    """

    return (
        f"Задание «{title}» выполнено: пайплайн собирается и воспроизводится, "
        "результаты сходятся с тем, что заявлено в выводах.\n\n"
        "Что стоит доработать:\n"
        "- вынести параметры эксперимента в конфиг — сейчас часть значений "
        "зашита в ячейках, и повторить прогон с другими числами тяжело;\n"
        "- добавить проверку на пустую выборку перед обучением: на текущих "
        "данных она не срабатывает, но падение здесь будет неочевидным;\n"
        "- в выводах опереться на числа из таблицы, а не на общее впечатление.\n\n"
        "В следующей работе попробуйте сравнить две конфигурации между собой — "
        "с таким пайплайном это уже почти бесплатно."
    )
    ...
    *,
    ...
    for index, student in enumerate(students):
        ...
        review = Review(
            ...
            final_feedback=_demo_feedback(spec["title"]),
...
def seed_demo(db: Session) -> None:
    ...
    assignment = Assignment(
        ...
        authoring={"topic": "Инструменты ML-инженера"},
        statement=(

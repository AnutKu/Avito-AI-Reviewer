"""Детерминированные демо-фикстуры для сидирования — и только для него.

Живого пути через этот модуль больше нет: и ревью, и блиц-вопросы делает
модель. Фикстуры остаются, чтобы в пустой демо-базе было что показать; они
помечены `demo_data: true` в `raw_result`, и интерфейс их так и называет —
«Демо-фикстура», с запретом перезапуска.
"""

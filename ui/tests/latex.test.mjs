...
describe('где формула начинается', () => {
  ...
  it('одиночная переменная в долларах — формула, а не цена', () => {
    assert.equal(
      renderMarkdown('Обозначим $n$ — число прогонов.'),
      '<p>Обозначим <span class="md-math">n</span> — число прогонов.</p>',
    )
  })

  it('цена перед формулой не съедает её доллар', () => {
    // Пара «$100, а метрика $» формулой не является, и поиск обязан
    // продолжиться со второго доллара — иначе настоящая формула следом
    // остаётся на экране долларами.
    const html = renderMarkdown('Бюджет $100, а метрика $R^2$.')

    assert.equal(html, '<p>Бюджет $100, а метрика <span class="md-math">R<sup>2</sup></span>.</p>')
  })
